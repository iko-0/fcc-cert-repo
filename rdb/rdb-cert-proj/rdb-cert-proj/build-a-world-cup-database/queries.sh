...
echo "$($PSQL "SELECT MAX(max) FROM (SELECT name, MAX(winner_goals) FROM teams LEFT JOIN games ON games.winner_id = teams.team_id GROUP BY name) AS max_goals")"
...
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
